...
- A pirate crew of 45 mates finds a treasure worth 15000€, they split it evenly. How much booty does each pirate receive?
...
- On the third day, you buy 45 cookies
- On the fourth day, you give 32 cookies to your friend
...
Values have *types*: "what kind of data is this?". By extension, we say that expressions (which evaluate to values), and variables (which contain values) also have types.
...
It means: "If the expression is true, then execute the block of code, otherwise skip it."
...
Given a variable `n` that contains a number, write a program that outputs "n is between 20 and 200" if `n` is indeed bigger than 20 and smaller than 200.
...
Given a variable `country` containing a string, print "Wine and cheese" if `country` is France, and print "Some other food" otherwise.
...
It means: "If condition1 is true, execute block 1. If it's false, but condition2 is true, exectute block 2. If that's also false, but condition 3 is true, execute block 3. If they were all false, just skip all the blocks."

---

# Mini exercise: `elseif`

Given a variable `number` containing a number, write different outputs depending on whether:
...
- For all numbers from 1 to 100, print whether they are smaller than 25, between 25 and 75, or bigger than 75.
...
In a `for` loop, we can also specify a step: 
...
- Can we split 77 toys in 6 children evenly? If not, how many toys are left?
- I am facing North, and make 23 quarter turns to the right. What direction am I facing now?
- If today is Tuesday, what day is it in 200 days?
...
- find a way to handle the last 2 verse, which are slightly different than the rest
...
- modify it to also print "Fizz" when the number is a multiple of 3
- modify it to also print "Buzz" when the number is a multiple of 5
- complete the exercise

**Hint:** don't hesitate to use debug instructions to check what is going on. Typically, `print(i)` in a `for` loop, to know what iteration you are at.

---

# Prime numbers

A prime number is an integer divisible only by 1 and itself. The first few prime numbers are:
...
- write a program that checks if a number `N` is a prime number
- put this into a loop

**Reminder:** a is divisible by b when `a % b == 0`

**Hint:** a number is *not* prime if it is a multiple of 2. Not either if it is a multiple of 3. Or 4. Or 5. And so on. It is not prime if it is a multiple of *anything* (except 1 and itself).
...
To pay for your retirement, you put 500 euros every month in a saving account that grows by 8% per year, compounded every month. Write a program that simulates the content of your savings account during the next 50 years.
