...
- find a way to handle the last 2 verses, which are slightly different than the rest
...
- modify it to *not* print the number when printing "Fizz" or "Buzz"
...
**Hint:** a number is *not* prime if it is a multiple of 2. Not either if it is a multiple of 3. Or 4. Or 5. And so on. It is not prime if it is a multiple of *anything* (except 1 and itself). And a yes/no question tends to translate as a boolean.
