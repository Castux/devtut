...
- Write a function `average_age(clients)` that returns the average age of the clients
	- hint: you can dedue the age from the birthday
	- reminder, use `tonumber()` to convert a string into the number it represents
...
- Using this dummy email function:
	...
	```
	write a function that sends a personalised "Happy birthday" email to everyone whose birthday it is today.
	- The function `os.date()` returns a string that contains the current date