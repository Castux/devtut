...
**Note:** Sublime Text doesn't handle input. `io.read()` systematically returns nil. We need to run the examples from the terminal.
...
- at first: pick a random free cell
- then: if a cell would give the win, play there
