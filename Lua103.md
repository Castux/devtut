...
Run `generate_text(n)` again now that you have punctuation.
...
- our corpus has 650313 words, so roughly as many sequences of three words
- each time we generate the next word, we need to check the whole array of sequences to find one that match our two previous words
- on average ~650000 checks per word
- to generate a text of 100000 words, that makes about 65 billions checks!
