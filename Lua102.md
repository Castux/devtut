...
A function can have a *return value* (output), that is a result that is *returned* to the caller. We use the keyword `return`, followed by an expression:
...
We say that the function "returns nil".
...
Complete documentation in the [reference](http://www.lua.org/manual/5.1/).
...
**Note:** in small programs, with only a few variables, OK to use globals.
