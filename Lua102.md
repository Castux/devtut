...
- so far, each variable contains *one* value
...
Can use any expression as index, as long as it is an integer:
...
- write a function which, given an array, prints all its elements (one by line)
- write a function which, given an array and another parameter, sets all the arrays elements to that parameter
- write a function equivalent to `table.concat()`: it should return a string of all the array's elements, with a separator string between all of them
