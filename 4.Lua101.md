...
The do repeat loop: sometimes practical do avoid repeating statements, or if it is easier to formulate an end condition than a while condition:
...
The `break` statement: "abort, abort!!". Exits a loop (while, repeat or for) early:
...
Always breaks the innermost loop:
