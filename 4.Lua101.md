...
Conditional statements: `if then else`. Lets you choose whether a *block* of code will be executed, according to a contition.
...
A sequence of integers starting with two ones, and where each number thereafter is the sum of the two previous ones in the sequence.
...
Write a program that recognizes if a string is a palindrome. To simplify, assume there is no capitals, punctuations or spaces, so that the program would only need recognize "amanaplanacanalpanama" as a palindrome.
